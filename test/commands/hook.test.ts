...
describe("every failure mode is silence", () => {
  ...
  test("tool-use, removed along with its activity log, is silent", () => {
    // A settings.json written before `empo update` still calls it on every Read.
    const read = {
      cwd: repo,
      tool_name: "Read",
      tool_input: { file_path: join(repo, "README.md") },
    };
    expect(hookAnswer("tool-use", read, { repo })).toBeNull();
  });
