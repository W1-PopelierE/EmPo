 ...
 * the `PreToolUse` groups match on tool name. `pre-commit` gets
...
function reviewSkill(config: EmpoConfig): string[] {
  return [
    ...
    "",
    // Which of the three the user wants is not derivable from anything the agent can read: the
    // narrowing is right or wrong depending on what the author did since the last round, and only
    // they know that. Asking costs one round trip; guessing costs a whole review of the wrong
    // subject, which is the failure this section already exists to prevent.
    ...
    "",
    // The failure this replaces: the gate prints nine verified findings and the agent starts
    // fixing all nine, having been asked for a review. A verified finding is a true statement
    // about the code, which is not the same thing as work the author wants done now, and the
    // review's own report ends at a verdict for exactly that reason.
    ...
    "",
    // Which two of the three apply is not a judgement call: a review of somebody else's pull
    // request that edits their branch instead of commenting on it has stopped being a review, and
    // offering "post a comment" on a local diff with no pull request offers something that cannot
    // be done. The brief already says which of the two this is.
