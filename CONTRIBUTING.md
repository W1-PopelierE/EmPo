...
- Biome: 2-space indent, **line width 100**, double quotes, `organizeImports` on. `fixtures`,
  `src/packs/*/fixtures`, `examples`, `.claude`, `.codex` and `.empo` are excluded, because a fixture is data
...
per command surface. `.claude/settings.json` wires three Claude-only `empo hook` entries
(SessionStart, PreToolUse on `Edit|Write`, PreToolUse on `Bash`). The `.claude/skills/empo-*` and
`.codex/skills/empo-*` files, plus the `AGENTS.md` managed block, are **generated by `empo update`**:
edit `.empo/config.json` and rerun, never the generated files. `empo init` creates the same host
wiring for a new project.
