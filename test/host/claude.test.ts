  ...
  test("replaces an entry an older version wrote instead of doubling it", () => {
    ...
    const stale = {
      hooks: {
        ...
        // The activity hook, which EmPo no longer wires at all: it goes, event and all.
        PostToolUse: [
          { matcher: "Read", hooks: [{ type: "command", command: "empo hook tool-use" }] },
        ],
      },
    };

    const merged = text(json(stale), BARE_HOOKS);
    const written = commands(merged);

    expect(written).toEqual(commands(text(null, BARE_HOOKS)));
    expect(written).not.toContain("empo hook pre-edit");
    expect(written).not.toContain("empo hook tool-use");
    expect(merged).not.toContain("PostToolUse");
  });
